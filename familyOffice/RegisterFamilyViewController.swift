            ...
            let uploadTask = storageRef.child(key).child("images").child("\(imageNasme).png").put(uploadData, metadata: nil) { metadata, error in
                if (error != nil) {
                    ...
                    if let downloadURL = metadata?.downloadURL()?.absoluteString {
                        self.activityILoad.stopAnimating()
                        ...
                        Utility.Instance().gotoView(view: "HomeView", context: self.self)
                    }
